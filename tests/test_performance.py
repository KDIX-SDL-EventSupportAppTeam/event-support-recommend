...
#
# 閾値はサーバー側タイムアウト **1000ms**（PURPOSE.md 前提6）に置く。
# ここを超えたら本番では必ず切られてフォールバックに落ちるので、
# 予算内かどうかの議論を待たず「異常」と言い切れる。予算 600ms 自体の
# 担保は実行時ガード（R-3。段3結線後）の役目であって単体テストではない。
#
# 実測は候補100件・規則100本で 1.4〜2.6ms（開発機）。閾値まで約400倍の余裕があり、
# 負荷のかかった CI でも揺れで赤くなることはない。一方で100倍規模の
# 性能退行は捕まえられる。**この余裕を根拠に閾値を緩めないこと**（緩めると何も検出しない）。
_ABSURDLY_SLOW_SEC = 1.0
