...
- **ブランチ名は半角英数字とハイフンだけで書く。日本語を使わない。**
  コミットメッセージと PR は日本語だが、**ブランチ名は例外**

> 日本語のブランチ名は URL エンコードされて各所で読めなくなり、
> リネームも安全に行えない（GitHub のブランチリネーム API が
> 開いている PR を追従させず、PR が閉じられる事故が起きた）。
